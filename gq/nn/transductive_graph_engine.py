...
from pyblaze.nn.engine.base import GpuSpec, _strip_metrics
from pyblaze.utils.torch import gpu_device, _recursive_apply

from gq.utils import apply_mask
...
class TransductiveGraphEngine(Engine):
    ...
    def supports_multiple_gpus(self):  # type: ignore
        return False

    def before_epoch(self, current, num_iterations):
        self.current_it = 0

    def after_batch(self, *_):  # type: ignore
            ...
            if val_data is not None and do_val:
                ...
                if eval_train:
                    ...
                    epoch_metrics = {
                        ...
                        **{
                            f"train_{k}": v
                            for k, v in eval_metrics_train.items()
                            if v is not None and not np.isnan(v)
                        },
                    }
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        results_with_val = self._aggregate_metrics_with_val(evals, metrics_with_val)
        generic_results = self._aggregate_generic_metrics(
            evals,
            generic_metrics,
            data.dataset,
        ...
        for s in self.splits:
            results[s] = {
                **results[s],
                **results_with_val.get(s, {}),
                **generic_results.get(s, {}),
            }
    ...
    def evaluate_target_and_ood(
        ...
        **kwargs,
        ...
        results_with_val = self._aggregate_metrics_with_val(evals, metrics_with_val)
        generic_results = self._aggregate_generic_metrics(
            evals, metrics, data, old_results=old_results
        )
    ...
    ################################################################################
    def predict(
        self, data, callbacks=None, gpu: GpuSpec = "auto", parallel=True, **kwargs
    ):
        if callbacks is None:
            callbacks = []
        ...
        for s in self.splits:
            if s == "all":
                if split_prefix in ("ood", "id"):
                    _y_hat, _y = apply_mask(data, y_hat, split_prefix)  # type: ignore
                else:
                ...
                _y_hat, _y = apply_mask(data, y_hat, _s)  # type: ignore
    ...
    def predict_batch(self, data, **kwargs) -> Prediction:
    ...
    def collate_predictions(self, predictions: list[Prediction]) -> list[Prediction]:  # type: ignore
        return predictions

    ################################################################################
    ### UTILITY FUNCTIONS
    ################################################################################
    def _metric_to_dict(self, metric, metric_key: str) -> dict[str, Any]:
        metric = self._process_metric(metric)
        if isinstance(metric, dict):
            res = {}
            for k, v in metric.items():
                submetric_key = f"{metric_key}_{k}" if k != "" else metric_key
                res.update(self._metric_to_dict(v, submetric_key))
            return res
    ...
    def _aggregate_metrics(self, evals, metrics):
        ...
        for s in self.splits:
            ...
            for metric_key, metric in metrics.items():
                metric_results[s].update(
                    self._metric_to_dict(metric(y_hat, y), metric_key)
                )
        return metric_results

    def _aggregate_metrics_with_val(self, evals, metrics):
        metric_results = {}
        ...
        y_hat_val, y_val = evals["val"]
        y_hat, y = evals["test"]
        metric_results["test"] = {}
        for metric_key, metric in metrics.items():
            metric_results["test"].update(
                self._metric_to_dict(metric(y_hat, y, y_hat_val, y_val), metric_key)
            )

        return metric_results

    def _aggregate_generic_metrics(
        self,
        evals,
        metrics,
        data,
        ...
        assert (
            "all" in self.splits
        ), "Generic Metrics can only be computed for unmasked data."

        metric_results = {s: {} for s in self.splits}
        y_hat, _ = evals["all"]
        ...
        for metric_key, metric in metrics.items():
            if old_results is not None:
                evaluated_submetrics = set(
                    [
                        k[len(split) + len(metric_key) + 2 :]
                        for k in old_results.keys()
                        for split in self.splits
                        if k.startswith(f"{split}_{metric_key}")
                    ]
                )
            else:
                evaluated_submetrics = None
            result = metric(
                y_hat,
                data,
                self.splits,
            ...
            for k, r in result.items():
                metric_results[k].update(self._metric_to_dict(r, metric_key))
    ...
    def _aggregate_metrics_ood(self, evals, evals_ood, metrics_ood):
        ...
        for s in self.splits:
            ...
            for metric_key, metric in metrics_ood.items():
                metric_results[s].update(
                    self._metric_to_dict(metric(y_hat, y, y_hat_ood, y_ood), metric_key)
                )

        return metric_results

    def _get_evals(self, data, callbacks=None, gpu: GpuSpec = "auto", **kwargs):
        if callbacks is None:
            callbacks = []
