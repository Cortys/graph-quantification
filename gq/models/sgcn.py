...
import copy
from gq.experiments.dataset import ExperimentDataset
import torch
...
import gq.nn as unn
from gq.layers import GCNConv
from gq.nn.loss import categorical_entropy_reg
from gq.utils import Prediction, apply_mask
from gq.utils import RunConfiguration, ModelConfiguration, DataConfiguration
from gq.utils import TrainingConfiguration
from gq.nn import loss_reduce
...
class SGCN(Model):
    ...
    def loss(self, prediction: Prediction, data: Data) -> Dict[str, Tensor]:
        ...
        if self.params.use_kernel:
            assert isinstance(self.alpha_prior, Tensor)
    ...
    def create_storage(
        ...
        if run_cfg.job == "train":
            ...
            if data_cfg.ood_flag and data_cfg.ood_type == "leave_out_classes_evasion":
                ...
                kernel.create_storage(
                    run_cfg, data_cfg, gdk_config, TrainingConfiguration()
                )
            ...
            if model_cfg.teacher_training:
                ...
                gcn_train_config.set_values(
                    lr=0.01, weight_decay=0.0001, epochs=100_000, stopping_patience=50
                )
                ...
                if (
                    data_cfg.ood_flag
                    and data_cfg.ood_type == "leave_out_classes_evasion"
                ):
                    dataset = ExperimentDataset(data_cfg)
                    with torch.no_grad():
